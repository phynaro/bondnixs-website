...
import { solutionsContentAPI } from '../../services/api'

const SolutionsContentList = () => {
  ...
  const fetchContent = async () => {
    try {
      const response = await solutionsContentAPI.getAllContentAdmin()
  ...
  const handleDelete = async (id, title) => {
    if (window.confirm(`Are you sure you want to delete "${title || 'this content'}"? This action cannot be undone.`)) {
      try {
        await solutionsContentAPI.deleteContent(id)
  ...
  const handleTogglePublish = async (id) => {
    try {
      const response = await solutionsContentAPI.togglePublish(id)
  ...
  const filteredContent = content.filter(item => {
    return filterType === 'all' || item.content_type === filterType
  })
    ...
    <div>
      <div className="mb-8">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Solutions Content</h1>
            <p className="mt-2 text-gray-600">Manage solutions page content sections</p>
          </div>
          <Link
            to="/admin/solutions-content/new"
                  ...
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex justify-end space-x-2">
                        <Link
                          to={`/admin/solutions-content/edit/${item.id}`}
...
export default SolutionsContentList
